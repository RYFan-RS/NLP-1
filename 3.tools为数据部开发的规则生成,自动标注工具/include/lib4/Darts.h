/*
  ATLAN C++ Library: Darts.h -- Double-Array Trie System
  ...
  Author: taku@chasen.org
  Modified by: atlantise522@gmail.com
*/
#ifndef __ACL__DARTS_H__
#define __ACL__DARTS_H__
...
//Darts Version 0.32
namespace acl{ namespace Darts{

  template <class T>
  void read_static(const char **ptr, T& value) {
    const char *r = *ptr;
	*ptr += sizeof(T);
    memcpy(&value, r, sizeof(T));
  }

  template <class T> inline T _darts_max(T x, T y) { return(x > y) ? x : y; }
  template <class T> inline T* _resize(T* ptr, size_t n, size_t l, T v) {
    T *tmp = new T[l];
	size_t i;
    for (i = 0; i < n; ++i) tmp[i] = ptr[i];
    for (i = n; i < l; ++i) tmp[i] = v;
    delete [] ptr;
    return tmp;
  }
  ...
  public:
    ...
    int open(const char *file,
      ...
      if (size_ != fread(reinterpret_cast<unit_t *>(array_),
    ...
    int save(const char *file,
      ...
      FILE *fp = fopen(file, mode);
      if (!fp) return -1;
      if (size_ != fwrite(reinterpret_cast<unit_t *>(array_),
    ...
    int gzopen(const char *file,
      ...
      size_ = size;
      if (!size_) {
        if (-1L != static_cast<long>(fseek(fp, -8, SEEK_END))) {
          char buf[8];
          if (fread(static_cast<char*>(buf),
                         1, 8, fp) != sizeof(buf)) {
            fclose(fp);
            return -1;
          }
  ...
  private:
    ...
    size_t insert(const std::vector <node_t> &siblings) {
      ...
      size_t begin = 0;
      size_t pos   = _darts_max((size_t)siblings[0].code + 1, next_check_pos_) - 1;
      ...
      next:
        ...
        if (alloc_size_ <= (begin + siblings[siblings.size()-1].code))
          resize(static_cast<size_t>(alloc_size_ *
                                     _darts_max(1.05, 1.0 * key_size_ / progress_)));
...
}}//namespace Darts //namespace acl
#endif //__ACL__DARTS_H__
