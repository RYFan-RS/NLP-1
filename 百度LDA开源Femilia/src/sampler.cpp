...
void GibbsSampler::sample_doc(LDADoc& doc) {
    int new_topic = -1;
    for (size_t i = 0; i < doc.size(); ++i) {
		//qiu:LDA文档存储基本单元，包含词id以及对应的主题id
        new_topic = sample_token(doc, doc.token(i));
		//qiu:修改单词所对应的主题，会对每一个主题总出现的次数进行修改		
        doc.set_topic(i, new_topic);
    }
}
...
int GibbsSampler::sample_token(LDADoc& doc, Token& token) {
    ...
    for (int t = 0; t < num_topics; ++t) {
		//qiu:狄利克雷分布:alpha,beta,即你从狄利克雷分配中以某一个概率选了一个分布
		//qiu:本次采样对应主题出现的总次数+_model->alpha()
        dt_alpha = doc.topic_sum(t) + _model->alpha();
		//qiu:某一个单词token.id对应的主题t出现的次数
        wt_beta = _model->word_topic(token.id, t) + _model->beta();
		//qiu:总的主题数为2000，beta_sum为词汇总数*0.01,topic_sum(t)对应的每一维主题的计数总和
		...
		//qiu:wt_beta / t_sum_beta_sum对应的是百度news模型中,求单个单词出现在某一个主题的概率
		//qiu:dt_alpha / t_sum_beta_sum对应的是当前采样中,某一个主题出现的概率。
