 ...
 private:
  void Init(const string& dict_path, const string& user_dict_paths, UserWordWeightOption user_word_weight_opt) {
    ...
    if (user_dict_paths.size()) {
	  ...
	  LoadUserDictEx(user_dict_paths);
    }
  ...
  void LoadUserDictEx(const string& filePaths) {
	acl::File WordsFile;
	WordsFile.setCommentMarks("#");
	WordsFile.read(filePaths);

	for(size_t j=0; j<WordsFile.size(); j++){
      DictUnit node_info;

	  WordsFile[j].strip();
      MakeNodeInfo(node_info, WordsFile[j], 
      user_word_default_weight_, UNKNOWN_TAG);
  ...
  void LoadUserDict(const string& filePaths) {
    ...
    for (size_t i = 0; i < files.size(); i++) {
      ...
      for (; getline(ifs, line); lineno++) {
        ...
        buf.clear();
        //Split(line, buf, " "); 单词可能有空格,比如IPHONE 5S. 修改单词,词频和词性 分隔符为\t
        Split(line, buf, "\t");
        DictUnit node_info;
        if(buf.size() == 1){
